...
Este guia publica `/api/*` em `https://daese.me/api/*` e encaminha para o bot local em `http://127.0.0.1:5056`.
...
```bash
cloudflared tunnel route dns cornwall-bot-api api.daese.me
```
...
3. No DevTools, as chamadas devem ir para `https://daese.me/api/...`.

## 4. Troubleshooting rapido

- `502 Falha no proxy Cloudflare`: tunnel fora do ar ou `api.daese.me` nao apontando para tunnel.
